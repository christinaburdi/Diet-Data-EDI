...
#check that the total number of prey column equals the sum of all the prey columns

totpreycheck = numball_format %>% 
  mutate(totprey = rowSums(across(c(`Acanthocyclops spp`: `Unid mysids`)))) %>% 
  mutate(sumcheck = if_else(`TotalNumberOfPrey` == totprey, "Y", "N")) %>% #add column to compare two columns
  filter(sumcheck !="Y") %>% 
  pivot_longer(cols = `Acanthocyclops spp`: `Unid mysids`, 
               names_to = "PreyCategory", 
               values_to = "PreyNumber") %>% 
  select(UniqueID, DietStudy, LogNumber, PreyCategory, PreyNumber, TotalNumberOfPrey, totprey, sumcheck) %>% 
  filter(PreyNumber != 0)
